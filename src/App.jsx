...
export default function App() {
  ...
  return (
    <div style={S.root}>
      ...
      {view === "home"        && <HomeView setView={setView} currentUser={currentUser} setCurrentUser={setCurrentUser} users={users} persistUsers={persistUsers} adminPw={adminPw} setAdminMode={setAdminMode} showToast={showToast} announcement={announcement} />}
...
function HomeView({ setView, currentUser, setCurrentUser, users, persistUsers, adminPw, setAdminMode, showToast, announcement }) {
...
// ─── ADMIN ────────────────────────────────────────────────────────────────────
function AdminView({ matches, persistMatches, bets, users, persistUsers, persistBets, groups, persistGroups, bonus, persistBonus, redemptions, persistRedemptions, convRate, persistConvRate, adminPw, setAdminPw, announcement, persistAnnouncement, showToast }) {
              ...
              : <div style={{display:"flex",flexDirection:"column",gap:8,padding:"12px",background:"rgba(192,57,43,0.07)",borderRadius:6,border:"1px solid rgba(192,57,43,0.2)"}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <button style={S.btnDanger} onClick={async()=>{
                      ...
                      await Promise.all([9001,9002,9003,9004,9005].map(id => deleteOne(KEYS.matches, String(id))));

                      // Retirer les matchs de test de l'état local (sans toucher aux vrais)
                      const realMatches = matches.filter(m => ![9001,9002,9003,9004,9005].includes(m.id));
