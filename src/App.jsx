...
function spentPoints(uid, redemptions) {
  return Object.values(redemptions[uid]||{}).filter(r=>r.status==="validated").reduce((acc,r)=>acc+r.pts,0);
}
  ...
  return (
    <div style={S.page}>
      ...
      {filtered.map(match => {
        ...
        return (
          <div key={match.id} style={closed ? {...S.matchCard,...S.matchCardClosed} : S.matchCard}>
...
function LeaderboardView({ matches, bets, users, bonus, redemptions, currentUser }) {
  ...
  return (
    <div style={S.page}>
      ...
      {kickedOff.length > 0 && (
        <div style={S.section}>
          <h3 style={S.sectionTitle}>Paris révélés — matchs commencés</h3>
          {kickedOff.map(m => (
            <div key={m.id} style={S.matchCard}>
              <div style={S.matchRow}>
                <strong style={{flex:1,textAlign:"right",fontFamily:"'Lexend',sans-serif",fontSize:13}}>{m.home}</strong>
                {m.finished
                  ? <span style={S.resultBadge}>{m.homeScore} - {m.awayScore}</span>
                  : <span style={{...S.resultBadge,background:"#e0dcd4",color:"#6b6b6b",fontSize:11}}>En cours…</span>
                }
                <strong style={{flex:1,textAlign:"left",fontFamily:"'Lexend',sans-serif",fontSize:13}}>{m.away}</strong>
              </div>
              <div style={S.betsReveal}>
                {Object.entries(users).map(([uid,u]) => {
                  const bet = bets[uid]?.[String(m.id)];
                  if (!bet) return null;
                  const pts = calcPoints(bet, m);
                  return (
                    <div key={uid} style={S.betRevealRow}>
                      <div style={{...S.avatarSm,background:u.color}}>{u.name[0]}</div>
                      <span style={S.betRevealName}>{u.name}</span>
                      <span style={S.betRevealScore}>{bet.homeScore}-{bet.awayScore}</span>
                      <span style={pts===4?S.ptsBadgeGold:pts===1?S.ptsBadgeSilver:S.ptsBadgeGrey}>{pts===4?"⭐4":pts===1?"✓1":"✗0"}</span>
                    </div>
                  );
                })}
              </div>
...
function SoldeView({ currentUser, matches, bets, bonus, redemptions, persistRedemptions, convRate, showToast }) {
  ...
  const statusLabel = s => s === "validated" ? "✅ Validée" : s === "refused" ? "✗ Refusée" : "⏳ En attente";
      ...
      {myRedemptions.length > 0 && (
        <div style={S.section}>
          ...
          {myRedemptions.map(([rid, r]) => (
            <div key={rid} style={{...S.matchCard,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              <div style={{flex:1}}>
                <div style={{fontFamily:"'Lexend',sans-serif",fontWeight:700,fontSize:14}}>{r.pts} pts → {r.euros}€</div>
                <div style={{fontSize:12,color:"#6b6b6b",fontStyle:"italic"}}>{r.reason} · {r.date}</div>
              </div>
...
// ─── ADMIN ────────────────────────────────────────────────────────────────────
function AdminView({ matches, persistMatches, bets, users, persistUsers, persistBets, groups, persistGroups, bonus, persistBonus, redemptions, persistRedemptions, convRate, persistConvRate, adminPw, setAdminPw, showToast }) {
  ...
  const finishMatch = async id => {
    ...
    await persistMatches(matches.map(m => m.id===id ? {...m,homeScore:h,awayScore:a,finished:true} : m));
    showToast("Résultat enregistré 🏁");
  };

  const resetMatch = async id => {
    await persistMatches(matches.map(m => m.id===id ? {...m,homeScore:null,awayScore:null,finished:false} : m));
              ...
              <>
                ...
                {allDone.length > 0 && (
                  <>
                    ...
                    {allDone.map(r => (
                      <div key={r.rid} style={{...S.adminMatchCard,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                        <div style={{...S.avatarSm,background:r.userColor}}>{r.userName[0]}</div>
                        <div style={{flex:1}}>
                          <span style={{fontFamily:"'Lexend',sans-serif",fontWeight:700,fontSize:13}}>{r.userName}</span>
                        ...
                        <span style={{borderRadius:3,padding:"2px 8px",fontSize:11,fontFamily:"'Lexend',sans-serif",fontWeight:700,
                          ...(r.status==="validated"?{background:"rgba(30,125,79,0.1)",color:"#1e7d4f"}:{background:"rgba(192,57,43,0.1)",color:"#c0392b"})}}>
                          {r.status==="validated"?"✅ Validée":"✗ Refusée"}
                        </span>
                      </div>
                    ))}
                  </>
