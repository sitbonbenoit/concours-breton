        ...
        <div>
          ...
          {pending.length > 0 && (
            <>
              ...
              {pending.map(m => {
                const KO_GROUPS = ["16e de finale","8e de finale","Quart de finale","Demi-finale","3e place","Finale"];
                ...
                <div key={m.id} style={S.adminMatchCard}>
                  ...
                  {isKO && (
                    <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}>
                      <span style={{fontSize:13,color:"#94a3b8"}}>Équipes :</span>
                      <input style={{...S.scoreInputSm,width:150}} placeholder="Équipe dom."
                        value={editScores[m.id]?.homeName??m.home}
                        onChange={e=>setEditScores(p=>({...p,[m.id]:{...p[m.id],homeName:e.target.value}}))} />
                      <span style={{color:"#475569",fontWeight:700}}>vs</span>
                      <input style={{...S.scoreInputSm,width:150}} placeholder="Équipe ext."
                        value={editScores[m.id]?.awayName??m.away}
                        onChange={e=>setEditScores(p=>({...p,[m.id]:{...p[m.id],awayName:e.target.value}}))} />
                      <button style={S.btnGhost} onClick={async()=>{
                        const hn=(editScores[m.id]?.homeName??m.home).trim();
                        const an=(editScores[m.id]?.awayName??m.away).trim();
                        if(!hn||!an) return showToast("Les noms d'équipes ne peuvent pas être vides","err");
                        await saveOneMatch(m.id,{home:hn,away:an});
                        showToast("Équipes mises à jour ✓");
                      }}>✏️ Renommer les équipes</button>
                    </div>
                  )}
